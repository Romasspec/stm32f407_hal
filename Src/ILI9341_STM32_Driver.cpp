...
static void ILI9341_SPI_TxBuffer(uint8_t *buffer, uint16_t len)
{
	...
	//while (*HSPI_INSTANCE.hdmatx->Instance->NDTR != 0);
...
{
	//while(!__HAL_SPI_GET_FLAG(HSPI_INSTANCE, SPI_FLAG_TXE));
	while (*HSPI_INSTANCE.hdmatx->Instance->NDTR != 0);        // proverka okonchaniya peredachi DMA
	HAL_GPIO_WritePin(LCD_DC_PORT, LCD_DC_PIN, GPIO_PIN_SET);	//data
	HAL_GPIO_WritePin(LCD_CS_PORT, LCD_CS_PIN, GPIO_PIN_RESET);	//select
...
void ILI9341_SetAddress(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
	...
	ILI9341_WriteBuffer(buffer_2, sizeof(buffer));
...
void ILI9341_SetRotation(uint8_t rotation)
{
	...
	case SCREEN_VERTICAL_1:
		ILI9341_WriteData(MADCTL_MY | MADCTL_BRG);
		LCD_WIDTH = ILI9341_SCREEN_HEIGHT;
		LCD_HEIGHT = ILI9341_SCREEN_WIDTH;
		break;
	case SCREEN_HORIZONTAL_1:
		ILI9341_WriteData(MADCTL_MY | MADCTL_MX | MADCTL_MV | MADCTL_BRG);
		LCD_WIDTH = ILI9341_SCREEN_WIDTH;
		LCD_HEIGHT = ILI9341_SCREEN_HEIGHT;
		break;
	case SCREEN_VERTICAL_2:
		ILI9341_WriteData(MADCTL_MX | MADCTL_BRG);
		LCD_WIDTH = ILI9341_SCREEN_HEIGHT;
		LCD_HEIGHT = ILI9341_SCREEN_WIDTH;
		break;
	case SCREEN_HORIZONTAL_2:
		ILI9341_WriteData(MADCTL_MV|MADCTL_BRG);
		LCD_WIDTH = ILI9341_SCREEN_WIDTH;
		LCD_HEIGHT = ILI9341_SCREEN_HEIGHT;
		break;
	default:
